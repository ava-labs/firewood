...
use crate::{
    ...
    path::{AsPathRef, IntoPathRef, PathRef},
    tries::{TrieEdgeState, TrieNode},
};
...
/// A boxed wrapper around [`MergeKeyProofError`] because it is a large error
/// type that negatively impacts generated code.
pub type MergeKeyProofError = Box<MergeKeyProofErrorBase>;

/// An error indicating that merging two key proof tries failed.
#[derive(Debug, Clone, PartialEq, Eq, Hash, thiserror::Error)]
pub enum MergeKeyProofErrorBase {
...
pub struct KeyProofTrieRoot<'a> {
    pub(super) partial_path: PathRef<'a>,
    pub(super) value_digest: Option<ValueDigest<&'a [u8]>>,
    pub(super) children: Children<Option<KeyProofTrieNode<'a>>>,
}

impl std::fmt::Debug for KeyProofTrieRoot<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KeyProofTrieRoot")
            .field("partial_path", &self.partial_path.display())
            .field("value", &DebugValue::new(self.value_digest.as_ref()))
...
#[derive(Debug)]
pub struct KeyRangeProofTrieRoot<'a> {
    root: Box<KeyProofTrieRoot<'a>>,
}

#[derive(Debug)]
pub(super) enum KeyProofTrieNode<'a> {
...
impl<'a> KeyProofTrieRoot<'a> {
    ...
    where
        T: AsRef<[N]> + ?Sized,
        N: Hashable<'a, FullPath: IntoSplitPath<Path: IntoPathRef<'a>>> + 'a,
    {
        proof
            .as_ref()
            .iter()
    ...
    {
        Box::new(Self {
            partial_path: node.full_path().into_split_path().into_path_ref(),
    ...
    fn merge(
        ...
        if !lower.partial_path.path_eq(&upper.partial_path) {
            return Err(Box::new(MergeKeyProofErrorBase::DisjointEdges {
        ...
        if lower.value_digest != upper.value_digest {
            return Err(Box::new(MergeKeyProofErrorBase::DuplicateKeys {
...
impl<'a> KeyRangeProofTrieRoot<'a> {
    ...
    pub fn new(
        lower: Option<Box<KeyProofTrieRoot<'a>>>,
        upper: Option<Box<KeyProofTrieRoot<'a>>>,
    ...
    /// bounding key proofs.
    #[must_use]
    pub const fn root(&self) -> &KeyProofTrieRoot<'a> {
        &self.root
    }

    pub(super) fn into_root(self) -> Box<KeyProofTrieRoot<'a>> {
        self.root
    }
}

impl KeyProofTrieNode<'_> {
    ...
    fn node(&self) -> Option<&dyn TrieNode> {
        match self {
            KeyProofTrieNode::Described { node, .. } => Some(node.upcast()),
            KeyProofTrieNode::Remote { .. } => None,
        }
    }

    fn as_edge_state(&self) -> TrieEdgeState<'_> {
        match self {
    ...
    fn merge_opt(
        ...
        match (lower, upper) {
            ...
            (Some(_), None) | (None, Some(_)) => {
                Err(Box::new(MergeKeyProofErrorBase::DisjointChildren {
    ...
    fn merge(
        ...
        match (lower, upper) {
            ...
            ) if lower_hash != upper_hash => Err(Box::new(MergeKeyProofErrorBase::DifferentHash {
                path: leading_path.cloned(),
                lower_hash,
                upper_hash,
            })),
...
impl TrieNode for KeyProofTrieRoot<'_> {
    fn partial_path(&self) -> PathRef<'_> {
        self.partial_path.as_path_ref()
    }
...
impl TrieNode for KeyRangeProofTrieRoot<'_> {
    fn partial_path(&self) -> PathRef<'_> {
        self.root.partial_path
    }

    fn value_digest(&self) -> Option<ValueDigest<&[u8]>> {
        self.root.value_digest.clone()
    }

    fn child_hash(&self, pc: PathComponent) -> Option<&HashType> {
        self.root.children[pc].as_ref().map(KeyProofTrieNode::hash)
    }

    fn child_node(&self, pc: PathComponent) -> Option<&dyn TrieNode> {
        self.root.children[pc]
            .as_ref()
            .and_then(KeyProofTrieNode::node)
    }
