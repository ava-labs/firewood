...
use crate::{
    Children, HashType, HashableShunt, IntoSplitPath, Node, NodeHashAlgorithm, Path, PathComponent,
    SplitPath,
};
...
pub fn hash_node(
    ...
    HashableShunt::from_node(path_prefix.as_components(), node).to_hash(node_hash_algorithm)
}
...
pub fn hash_preimage(
    ...
    HashableShunt::from_node(path_prefix.as_components(), node)
        .write(&mut buf, node_hash_algorithm);
...
impl<T: AsRef<[u8]>> ValueDigest<T> {
    ...
    pub fn verify(&self, expected: impl AsRef<[u8]>) -> bool {
        match self {
            ...
            Self::Hash(got_hash) => {
                ...
                *got_hash == HashType::Hash(Sha256::digest(expected.as_ref()).into())
            }
        }
    }
    ...
    /// instead of hashing.
    ///
    /// If Ethereum hashing is selected, this always returns the value as-is.
    pub fn make_hash(&self, node_hash_algorithm: NodeHashAlgorithm) -> ValueDigest<&[u8]> {
        match (node_hash_algorithm, self.as_ref()) {
            (NodeHashAlgorithm::MerkleDB, ValueDigest::Value(v)) if v.len() >= 32 => {
                use sha2::{Digest, Sha256};
                ValueDigest::Hash(HashType::Hash(Sha256::digest(v).into()))
            }
            (_, ValueDigest::Value(v)) => ValueDigest::Value(v),
            (_, ValueDigest::Hash(v)) => ValueDigest::Hash(v),
        }
    }
...
pub trait Preimage: std::fmt::Debug {
    ...
    fn to_hash(&self, node_hash_algorithm: NodeHashAlgorithm) -> HashType;

    /// Write this hash preimage to `buf`.
    fn write(&self, buf: &mut impl HasUpdate, node_hash_algorithm: NodeHashAlgorithm);
}

impl<T: Hashable> Preimage for T {
    fn to_hash(&self, node_hash_algorithm: NodeHashAlgorithm) -> HashType {
        crate::hashers::to_hash(node_hash_algorithm, self)
    }

    fn write(&self, buf: &mut impl HasUpdate, node_hash_algorithm: NodeHashAlgorithm) {
        crate::hashers::write_preimage(node_hash_algorithm, self, buf);
    }
}
