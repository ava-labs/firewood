        ...
        match res.cmp(&0) {
            Ordering::Less => {
                ...
                if matches!(err.kind(), ErrorKind::WouldBlock) {
                    ...
                    firewood_increment!(crate::registry::ring::EAGAIN_WRITE_RETRY_TOTAL, 1);
                    return Ok(0);
                }
...
impl<'batch, 'ring, I: Iterator<Item = QueueEntry<'batch>>> WriteBatch<'batch, 'ring, I> {
    ...
    fn enqueue_single_op(&mut self) -> EnqueueResult {
        ...
        if unsafe { self.sq.push(&sqe) }.is_err() {
            ...
            firewood_increment!(crate::registry::ring::FULL_TOTAL, 1);
            EnqueueResult::Full
        } else {
    ...
    fn flush_submission_queue(&mut self) -> io::Result<()> {
        ...
        loop {
            if self.sq.is_full() {
                ...
                firewood_increment!(crate::registry::ring::SQ_WAIT_TOTAL, 1);
            ...
            Ok(written) => {
                ...
                if !write_entry.buffer.is_empty() {
                    ...
                    if written != 0 {
                        ...
                        firewood_increment!(crate::registry::ring::PARTIAL_WRITE_RETRY_TOTAL, 1);
                    }
                }
                let carry;
