...
mod test_utils {
    ...
    impl SeededRng {
        ...
        /// # Panics
        ///
        /// Panics if the seed is not a valid `u64`.
        #[track_caller]
            ...
            std::env::var(Self::ENV)
                ...
                .map(|s| {
                    s.parse().unwrap_or_else(|err| {
                        panic!("{} must be a valid u64, got: {s}; {err}", Self::ENV)
                    })
                })
                .map(Self::new)
        }
        ...
        pub fn from_random() -> Self {
            let seed = rand::rngs::StdRng::from_os_rng().random();
            eprintln!(
    ...
    impl SeedableRng for SeededRng {
        ...
        fn from_seed(seed: Self::Seed) -> Self {
            Self(Rc::new(RefCell::new(StdRng::from_seed(seed))))
        }
    }
}
