...
impl NodeStore<Committed, FileBacked> {
    ...
    pub fn flush_nodes(&mut self) -> Result<NodeStoreHeader, FileIoError> {
        ...
        for node in unpersisted_iter {
            ...
            let (persisted_address, _) = allocator.allocate_node(serialized.as_slice())?;
            self.storage
        ...
        self.storage.write_cached_nodes(cached_nodes.into_iter().map(|(addr, node)| (*addr, node)))?;
...
impl NodeStore<Committed, FileBacked> {
    ...
    pub fn flush_nodes(&mut self) -> Result<NodeStoreHeader, FileIoError> {
        ...
        for node in unpersisted_nodes {
            ...
            shared_node.as_bytes(0, &mut serialized);
            let (persisted_address, _) = node_allocator.allocate_node(serialized.as_slice())?;
