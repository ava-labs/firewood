...
use crate::node::Node;
use crate::{Child, HashType, MaybePersistedNode, Path, SharedNode};
...
#[cfg(feature = "ethhash")]
pub(super) fn ethhash_classify_children<'a, S: NodeReader>(
    storage: &S,
            ...
            match child {
                ...
                Some(Child::MaybePersisted(maybe_persisted, h)) => {
                    ...
                    if let Some(addr) = maybe_persisted.as_linear_address() {
                        ...
                        if let Ok(node) = maybe_persisted.as_shared_node(storage) {
...
pub(super) fn hash_helper(
    ...
    #[cfg(feature = "ethhash")] storage: &impl NodeReader,
    ...
    if let Node::Branch(ref mut b) = node {
        ...
        let make_fake_root = if path_prefix.0.len().saturating_add(b.partial_path.0.len()) == 64 {
            ...
            let ClassifiedChildren {
                unhashed,
                mut hashed,
            } = ethhash_classify_children(storage, &mut b.children);
            ...
            if hashed.len() == 1 {
                ...
                let mut hashable_node = storage.read_node(addr)?.deref().clone();
        ...
        for (nibble, child) in b.children.iter_mut().enumerate() {
            ...
            let (child_node, child_hash) =
                hash_helper(child_node, path_prefix, make_fake_root, storage)?;
            #[cfg(not(feature = "ethhash"))]
            let (child_node, child_hash) = hash_helper(child_node, path_prefix)?;
