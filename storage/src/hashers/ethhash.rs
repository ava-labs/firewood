...
use crate::{
    BranchNode, HashType, Hashable, TrieHash, TriePath, ValueDigest, hashednode::HasUpdate,
    logger::trace,
};
...
pub(crate) fn to_hash<T: Hashable>(hashable: &T) -> HashType {
    ...
    write(hashable, &mut collector);

    trace!(
        "SIZE WAS {} {}",
        hashable.full_path().len(),
...
pub(crate) fn write<T: Hashable>(hashable: &T, buf: &mut impl HasUpdate) {
    let is_account = hashable.full_path().len() == 64;
    trace!("is_account: {is_account}");

    let child_hashes = hashable.children();
    ...
    if children == 0 {
        ...
        rlp.append(&&*nibbles_to_eth_compact(hashable.partial_path(), true));

        if is_account {
            // we are a leaf that is at depth 32
            match hashable.value_digest() {
                ...
                Some(ValueDigest::Hash(_)) => unreachable!("ethereum proofs do not hash values"),
                None => {
        ...
        let bytes = rlp.out();
        trace!("partial path {:?}", hashable.partial_path().display());
        ...
        let digest = (!is_account).then(|| hashable.value_digest()).flatten();
        ...
        let updated_bytes = if is_account {
            ...
            if let Some(ValueDigest::Value(rlp_encoded_bytes)) = hashable.value_digest() {
        ...
        let partial_path = hashable.partial_path();
