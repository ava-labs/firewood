    ...
    fn write(&self, buf: &mut impl HasUpdate) {
        ...
        if children == 0 {
            ...
            let updated_bytes = if is_account {
                ...
                if let ValueDigest::Value(rlp_encoded_bytes) =
                    self.value_digest().expect("must have a value")
                {
                    ...
                    let bytes = replace_hash(rlp_encoded_bytes, replacement_hash).unwrap_or_else(|| BytesMut::from(rlp_encoded_bytes));
...
fn replace_hash<T: AsRef<[u8]>, U: AsRef<[u8]>>(bytes: T, new_hash: U) -> Option<BytesMut> {
    ...
    let mut list = rlp::decode_list::<Vec<u8>>(bytes.as_ref());
