...
fn main() -> Result<(), WalError> {
    tokio_uring::start(async {
        ...
        let mut wal = loader.load(store, recover, 0).await?;
        for _ in 0..3 {
            let _ = test(
                ["hi", "hello", "lol"]
                    .iter()
        ...
        // let store = WalStoreImpl::new(wal_dir, false)?;
        // let mut wal = loader.load(store, recover, 100).await?;
        ...
        // }
        // for (rec, ans) in
        //     (wal.read_recent_records(100, &growthring::wal::RecoverPolicy::Strict)).await?
        //         .into_iter()
