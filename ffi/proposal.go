...
func (p *Proposal) Iter(key []byte) (*Iterator, error) {
	...
	return getIteratorFromIteratorResult(itResult, p.keepAliveHandle.db)
}
...
func (p *Proposal) Propose(batch []BatchOp) (*Proposal, error) {
	...
	return getProposalFromProposalResult(C.fwd_propose_on_proposal(p.ptr, kvp), p.keepAliveHandle.db, p.commitLock)
}
...
func getProposalFromProposalResult(result C.ProposalResult, db *Database, commitLock *sync.Mutex) (*Proposal, error) {
	switch result.tag {
	...
	case C.ProposalResult_Ok:
		...
		proposal := &Proposal{
			handle:     createHandle(body.handle, db, func(p *C.ProposalHandle) C.VoidResult { return C.fwd_free_proposal(p) }),
			root:       hashKey,
			commitLock: commitLock,
		}
		db.registerLiveHandle(&proposal.keepAliveHandle, proposal.handle.Drop)
