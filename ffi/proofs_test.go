...
func TestChangeProofMarshalFailsAfterVerify(t *testing.T) {
	...
	proposedChangeProof, err := dbB.VerifyAndProposeChangeProof(changeProof, rootA, rootAUpdated, nothing(), nothing(), changeProofLenUnbounded)
	r.NoError(err)
	t.Cleanup(func() { r.NoError(proposedChangeProof.Free()) })
...
func TestProposedChangeProofMarshal(t *testing.T) {
	...
	marshalledAfterPropose, err := proposedChangeProof.MarshalBinary()
	r.NoError(err)
