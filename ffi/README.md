...
The hash mode is selected at runtime, per database, via the node hash algorithm rather than a cargo feature. Choose it when you open the database: pass the algorithm to `New(dir, NodeHashAlgorithm, ...)` in Go or configure it and call `open(path, cfg)` in Rust. A single static library serves both modes, so no separate build is required.
