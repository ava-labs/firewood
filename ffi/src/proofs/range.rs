...
impl<'db> RangeProofContext<'db> {
    ...
    fn verify(
        ...
        // self-describing mode (like the code-iterator paths below). A
        // cross-mode forgery is still caught by hashing — a proof hashed under
        // the wrong scheme cannot reproduce `root`.
