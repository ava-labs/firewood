...
mod tests {
    ...
    fn test_inverted_range_rejected() {
        ...
        let result =
            verify_change_proof_structure(&proof, root2, Some(b"\xa0"), Some(b"\x10"), None);
    ...
    fn test_boundary_proof_unverifiable_via_structure() {
        ...
        let result = verify_change_proof_structure(
            &proof,
            root2,
    ...
    fn test_unexpected_start_proof_rejected() {
        ...
        // to validate against → BoundaryProofUnverifiable
        let result = verify_change_proof_structure(&proof, root2, None, Some(b"\xa0"), None);
    ...
    fn test_empty_end_root_with_batch_ops_rejected_partial() {
        ...
        let empty_root = firewood::api::HashKey::empty();
        let result =
            verify_change_proof_structure(&proof, empty_root, Some(b"\x10"), Some(b"\xa0"), None);
    ...
    fn test_empty_proof_rejected() {
        ...
        let _result = verify_change_proof_structure(
            &empty_proof,
            dummy_root.clone(),
            Some(b"\x10"),
            Some(b"\xa0"),
    ...
    fn test_start_key_larger_than_first_key() {
        ...
        let result = verify_change_proof_structure(&proof, root2, Some(b"\xff"), None, None);
    ...
    fn test_end_key_less_than_last_key() {
        ...
        let result = verify_change_proof_structure(&proof, root2, None, Some(b"\x01"), None);
    ...
    fn test_proof_larger_than_max_length() {
        ...
        let result = verify_change_proof_structure(&proof, root2, None, None, NonZeroUsize::new(1));
    ...
    fn test_divergence_at_depth_zero() {
        ...
        let verification = firewood::ChangeProofVerificationContext {
            end_root: root2,
    ...
    fn test_omitted_change_attack() {
        ...
        let mut shortened_ops: Vec<BatchOp<firewood::Key, firewood::Value>> = batch_ops.to_vec();
    ...
    fn test_omitted_change_attack_bounded() {
        ...
        if batch_ops.len() >= 2 {
            // Remove one op
            let mut shortened_ops: Vec<BatchOp<firewood::Key, firewood::Value>> =
                batch_ops.to_vec();
    ...
    // Cursor unit tests have been moved to firewood/src/merkle/tests/change.rs
    // where the ChangeProofCursor type now lives.
    ...
    fn test_spurious_batch_op_detected() {
        ...
        let mut tampered_ops: Vec<BatchOp<firewood::Key, firewood::Value>> =
    ...
    fn test_missing_delete_detected() {
        ...
        let tampered_ops: Vec<BatchOp<firewood::Key, firewood::Value>> = honest
            .batch_ops()
            .iter()
    ...
    fn test_forged_value_detected() {
        ...
        let forged_ops: Vec<BatchOp<firewood::Key, firewood::Value>> = honest
            .batch_ops()
            .iter()
            .map(|op| match op {
