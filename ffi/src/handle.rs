...
use firewood::{
    ...
    v2::api::{self, HashKey, KeyValuePairIter},
};

use crate::{BorrowedBytes, CreateProposalResult, KeyValuePair, ProposalExt, arc_cache::ArcCache};
...
impl DatabaseHandle {
    ...
    pub fn get_latest(&self, key: impl AsRef<[u8]>) -> Result<Option<Box<[u8]>>, api::Error> {
        let Some(root) = self.current_root_hash()? else {
            return Err(api::Error::HashNotFound {
                provided: HashKey::default(),
            });
        };
    ...
    pub fn get_from_root(
        ...
        let root: HashKey = root.as_ref().try_into()?;
...
/// A marker type that increments the `firewood.ffi.cached_view.hit` counter when dropped.
///
/// However, if the `miss` method is called, it will instead increment the
/// `firewood.ffi.cached_view.miss` counter and prevent the drop from incrementing
/// the hit counter.
...
impl<'db> ProposalExt<'db> for &'db crate::DatabaseHandle {
