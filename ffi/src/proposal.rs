...
/// An opaque wrapper around a Propoal that also retains a reference to the
...
impl ProposalHandle<'_> {
    ...
    pub fn commit_proposal(
        ...
        _ = handle.get_root(hash_key.clone().unwrap_or_default());

        let start_time = coarsetime::Instant::now();
        proposal.commit_sync()?;
        let commit_time = start_time.elapsed();

        // clear the cached view so that it does not hold onto the proposal
...
impl<'db> ProposalExt<'db> for &ProposalHandle<'db> {
    fn handle(&self) -> &'db crate::DatabaseHandle {
        self.handle
    }
