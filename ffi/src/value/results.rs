...
use crate::{
    ...
    OwnedKeyValuePair, OwnedRenderedMetrics, ProposalHandle, RangeProofContext, ReconstructedHandle,
    SyncHandle, SyncWorkItem,
};
...
impl From<Option<Result<(Key, Value), api::Error>>> for KeyValueResult {
    fn from(value: Option<Result<(Key, Value), api::Error>>) -> Self {
        match value {
            Some(value) => match value {
...
impl From<Result<Vec<(Key, Value)>, api::Error>> for KeyValueBatchResult {
    fn from(value: Result<Vec<(Key, Value)>, api::Error>) -> Self {
        match value {
            Ok(pairs) => {
