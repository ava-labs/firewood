...
func (db *Database) Update(keys, vals [][]byte) ([]byte, error) {
	if len(keys) != len(vals) {
		return nil, fmt.Errorf("keys and values must have the same length: %d != %d", len(keys), len(vals))
	}
	...
	keyValuePairs := newKeyValuePairs(keys, vals, &pinner)

	return fromHashResult(C.fwd_batch(db.handle, keyValuePairs))
}
...
func (db *Database) Propose(keys, vals [][]byte) (*Proposal, error) {
	...
	return fromProposalResult(C.fwd_propose_on_db(db.handle, keyValuePairs), db)
}
