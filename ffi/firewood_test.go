...
const (
	...
	numTestEntries        = 20
	halfTestItems         = 5
	numRevisionsToKeep    = 5
	numTestBatchOps       = 3
	numTestHandles        = 2
	numKVParts            = 2
	numReapingTestEntries = 40

	// Format string constants for test assertions
	fmtGet          = "Get(%d)"
	fmtCommit       = "Commit(%d)"
	fmtPropose      = "Propose(%d)"
	fmtBaseGet      = "base.Get(): %d"
	fmtProposeType  = "%T.Propose(...)"
	fmtRevisionType = "%T.Revision(...)"
...
func TestMain(m *testing.M) {
	...
	for _, kv := range debug {
		...
		case "cgocheck=1":
			hasCgoCheck = true
		case "cgocheck=0":
			if _, err := fmt.Fprint(os.Stderr, "GODEBUG=cgocheck=0; MUST be 1 for Firewood cgo tests"); err != nil {
				panic(err)
			}
			os.Exit(1)
		}
	...
	if !hasCgoCheck {
		...
		if err := os.Setenv("GODEBUG", strings.Join(debug, ",")); err != nil {
			if _, err := fmt.Fprintf(os.Stderr, `os.Setenv("GODEBUG", ...) error %v`, err); err != nil {
				panic(err)
			}
			os.Exit(1)
		}
	...
	if hashMode == "" {
		...
		if err != nil {
			if _, err := fmt.Fprintf(os.Stderr, "failed to infer hash mode %v\n", err); err != nil {
				panic(err)
			}
			os.Exit(1)
		}
	...
	if !ok {
		if _, err := fmt.Fprintf(os.Stderr, "unknown hash mode %q\n", hashMode); err != nil {
			panic(err)
		}
		os.Exit(1)
	}
...
func TestNodeHashAlgorithmValidation(t *testing.T) {
	...
	case ethereumOnly:
		...
		r.Error(errMerkledb)
		r.ErrorContains(errMerkledb,
			"node store hash algorithm mismatch: want to initialize with MerkleDB, but build option is for Ethereum")
	case merkledbOnly:
		r.NoError(dbMerkledb.Close(oneSecCtx(t)))
		r.Error(errEth)
		r.ErrorContains(errEth,
			"node store hash algorithm mismatch: want to initialize with Ethereum, but build option is for MerkleDB")
	case bothOrNeither:
...
func TestUpdateMultiKV(t *testing.T) {
	r := require.New(t)
	db := newTestDatabase(t)
	keys, vals, batch := kvForTest(numTestItems)
	_, err := db.Update(batch)
	r.NoError(err)
...
func TestTruncateDatabase(t *testing.T) {
	...
	// Insert some data.
	_, _, batch := kvForTest(numTestItems)
	_, err = db.Update(batch)
	r.NoError(err)
...
func kvForTest(num int) ([][]byte, [][]byte, []BatchOp) {
	...
	if err := sortKV(keys, vals); err != nil {
		panic(err)
	}
...
func TestInvariants(t *testing.T) {
	...
	got, err := db.Get([]byte(nonExistentKey))
	r.NoError(err)
	r.Empty(got)
}
...
func TestConflictingProposals(t *testing.T) {
	...
	for i, p := range proposals {
		for j := range numKeys {
			...
			r.NoError(err)
			r.Equal(valForTest(i*numKeys+j), got, fmtGet, i*numKeys+j)
		}
	}
	...
	for i := 1; i < numProposals; i++ {
		...
		r.Contains(err.Error(), errWrongParent, fmtCommit, i)
	}

	// After attempting to commit the other proposals, they should be completely invalid.
	for i := 1; i < numProposals; i++ {
		err := proposals[i].Commit()
		r.ErrorIs(err, errDroppedProposal, fmtCommit, i)
	}
	...
	for i := 1; i < numProposals; i++ {
		for j := range numKeys {
			got, err := proposals[i].Get(keyForTest(i*numKeys + j))
			r.ErrorIs(err, errDroppedProposal, fmtGet, i*numKeys+j)
			r.Empty(got, fmtGet, i*numKeys+j)
		}
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := range keys {
				...
				r.NoError(err, fmtGet, i)
				r.Empty(got, fmtGet, i)
			}
...
func TestDropProposal(t *testing.T) {
	...
	// Create a proposal with numTestItems keys.
	keys, _, batch := kvForTest(numTestItems)
	proposal, err := db.Propose(batch)
	...
	_, err = proposal.Get([]byte(nonExistentKey))
	r.ErrorIs(err, errDroppedProposal)
	_ = proposal.Root()
...
func TestProposeFromProposal(t *testing.T) {
	...
	keys1 := make([][]byte, numTestItems)
	vals1 := make([][]byte, numTestItems)
	keys2 := make([][]byte, numTestItems)
	vals2 := make([][]byte, numTestItems)
	for i := range keys1 {
...
func TestDeepPropose(t *testing.T) {
	...
	for i := range proposals {
		err := proposals[i].Commit()
		r.NoError(err, fmtCommit, i)

		for j := i * numKeys; j < (i+1)*numKeys; j++ {
			got, err := db.Get(keys[j])
			r.NoError(err, fmtGet, j)
			r.Equal(vals[j], got, fmtGet, j)
		}
	}
}
...
func TestDropProposalAndCommit(t *testing.T) {
	...
	r.ErrorIs(err, errDroppedProposal, fmtGet, 0)
...
func TestProposeSameRoot(t *testing.T) {
	...
	proposal1, err := db.Propose(batch[0:halfTestItems])
	r.NoError(err)
	proposal3Top, err := proposal1.Propose(batch[halfTestItems:numTestItems])
	r.NoError(err)
	// Create the second proposal chain.
	proposal2, err := db.Propose(batch[halfTestItems:numTestItems])
	r.NoError(err)
	proposal3Bottom, err := proposal2.Propose(batch[0:halfTestItems])
	r.NoError(err)
	// Because the proposals are identical, they should have the same root.

	// Create a unique proposal from each of the two chains.
	topKeys := make([][]byte, halfTestItems)
	topVals := make([][]byte, halfTestItems)
	for i := range topKeys {
		topKeys[i] = keyForTest(i + numTestItems)
		topVals[i] = valForTest(i + numTestItems)
	}
	bottomKeys := make([][]byte, halfTestItems)
	bottomVals := make([][]byte, halfTestItems)
...
func TestRevision(t *testing.T) {
	...
	// Create a proposal with numTestItems key-value pairs.
	proposal, err := db.Propose(batch)
	r.NoError(err)
	...
	for i := range keys {
		...
		r.NoError(err, fmtGet, i)
		r.Equal(valForTest(i), got, fmtGet, i)
	}

	// Create a second proposal with numTestItems key-value pairs.
	keys2 := make([][]byte, numTestItems)
	vals2 := make([][]byte, numTestItems)
	for i := range keys2 {
		keys2[i] = keyForTest(i + numTestItems)
		vals2[i] = valForTest(i + numTestItems)
	}
...
func TestRevisionOutlivesProposal(t *testing.T) {
	...
	keys, vals, batch := kvForTest(numTestEntries)
	_, err := db.Update(batch[:numTestItems])
	r.NoError(err)

	// Create a proposal with numTestItems key-value pairs.
	nKeys, nVals := keys[numTestItems:], vals[numTestItems:]
	proposal, err := db.Propose(batch[numTestItems:])
	r.NoError(err)
	root := proposal.Root()
...
func TestCommitWithRevisionHeld(t *testing.T) {
	...
	keys, vals, batch := kvForTest(numTestEntries)
	root, err := db.Update(batch[:numTestItems])
	r.NoError(err)
	...
	for i, key := range keys {
		...
		r.NoErrorf(err, fmtBaseGet, i)
		if i < numTestItems {
			r.Equalf(vals[i], val, fmtBaseGet, i)
		} else {
			r.Emptyf(val, fmtBaseGet, i)
		}
		val, err = rev.Get(key)
...
func TestRevisionOutlivesReaping(t *testing.T) {
	r := require.New(t)
	db := newTestDatabase(t, WithRevisions(numTestHandles))

	keys, vals, batch := kvForTest(numReapingTestEntries)
	firstRoot, err := db.Update(batch[:numTestItems])
	r.NoError(err)
	...
	secondRoot, err := db.Update(batch[numTestItems:numTestEntries])
	r.NoError(err)
	_, err = db.Update(batch[numTestEntries : numTestEntries+numTestItems])
	r.NoError(err)
...
func TestGetNilCases(t *testing.T) {
	...
	// Commit numTestItems key-value pairs.
	_, _, batch := kvForTest(numTestEntries)
	root, err := db.Update(batch[:numTestItems])
	r.NoError(err)
...
func TestEmptyProposals(t *testing.T) {
	...
	for i := range numProposals {
		...
		got, err := emptyProposals[i].Get([]byte(nonExistentKey))
		r.NoError(err, fmtGet, i)
		r.Empty(got, fmtGet, i)
	}
...
func TestFjallStore(t *testing.T) {
	...
	db, err := newDatabase(dbDir,
		WithRootStore(),
		WithRevisions(numRevisionsToKeep),
	)
	r.NoError(err)

	// Create and commit numTestItems proposals
	numRevisions := numTestItems
	key := []byte("root_store")
...
func TestCloseWithMultipleActiveHandles(t *testing.T) {
	...
	_, _, batch := kvForTest(numTestBatchOps)
...
func TestCloseSucceedsWhenHandlesDroppedInTime(t *testing.T) {
	...
	_, _, batch := kvForTest(numTestHandles)
	proposal1, err := db.Propose(batch[:1])
	r.NoError(err)
	proposal2, err := db.Propose(batch[1:2])
	r.NoError(err)
...
func TestDump(t *testing.T) {
	...
	dump, err := db.Dump()
	r.NoError(err)
	r.Contains(dump, dotDigraph, "dump should be in DOT format")
	...
	r.NoError(err)
	r.Contains(revisionDump, dotDigraph, "revision dump should be in DOT format")
	for _, val := range vals {
	...
	r.NoError(err)
	r.Contains(proposalDump, dotDigraph, "proposal dump should be in DOT format")
...
//revive:disable:line-length-limit // hex-encoded blockchain test data
var block307Items = `
...
func TestProposeOnProposalRehash(t *testing.T) {
	...
	b307k, b307v, err := parseKVFromDumpFormat(block307Items)
	r.NoError(err, "load 307")

	b308k, b308v, err := parseKVFromDumpFormat(block308Items)
