...
//!
//! Provides `write_to_vec` and `from_slice` for [`WitnessProof`] and
//! [`TruncatedTrie`], enabling wire transport over gRPC.
//!
...
impl WitnessProof {
    /// Serializes the witness proof into the provided byte vector.
    ///
    /// # Format
    ///
    /// - 8 bytes: magic `b"fwdwitns"`
    /// - 1 byte: version (0)
    /// - 32 bytes: `new_root_hash`
    /// - varint: `batch_ops` count, then each op
    /// - varint: `witness_nodes` count, then each node (path + serialized node)
    pub fn write_to_vec(&self, out: &mut Vec<u8>) {
        out.extend_from_slice(WITNESS_MAGIC);
        out.push(VERSION);
        out.extend_from_slice(self.new_root_hash.as_ref());
        ...
        // Witness nodes
        out.push_var_int(self.witness_nodes.len());
        for wn in &*self.witness_nodes {
            write_bytes(out, &wn.path);
            write_node(out, &wn.node);
        }
    }

    /// Deserializes a witness proof from a byte slice.
    ///
    /// # Errors
    ///
    ...
    pub fn from_slice(data: &[u8]) -> Result<Self, ReadError> {
        ...
        let magic = reader
            .read_chunk::<8>()
            .map_err(|err| err.set_item("witness magic"))?;
        ...
        if version != VERSION {
            return Err(reader.invalid_item("witness version", "0", version));
        }

        // Root hash
        let new_root_hash = reader.read_item::<TrieHash>()?;

        // Batch ops
        let num_ops = reader
            .read_item::<usize>()
            .map_err(|err| err.set_item("batch ops count"))?;
        let mut batch_ops = Vec::with_capacity(num_ops);
        for _ in 0..num_ops {
            let tag = reader.read_item::<u8>()?;
            match tag {
                BATCH_PUT => {
        ...
        let num_nodes = reader
            .read_item::<usize>()
            .map_err(|err| err.set_item("witness nodes count"))?;
        let mut witness_nodes = Vec::with_capacity(num_nodes);
        for _ in 0..num_nodes {
            let path = reader.read_item::<Box<[u8]>>()?;
            let node = read_node(&mut reader)?;
            witness_nodes.push(WitnessNode { path, node });
        }

        if !reader.remainder().is_empty() {
            return Err(reader.invalid_item(
                "trailing bytes",
                "no data after witness proof",
...
mod tests {
    ...
    use crate::remote::witness::generate_witness;
    ...
    #[test]
    fn test_witness_proof_roundtrip() {
        ...
        // Serialize
        let mut buf = Vec::new();
        witness.write_to_vec(&mut buf);
        assert!(!buf.is_empty());

        // Deserialize
        let deserialized = WitnessProof::from_slice(&buf).unwrap();

        // Verify fields match
        assert_eq!(deserialized.new_root_hash, witness.new_root_hash);
        assert_eq!(deserialized.batch_ops.len(), witness.batch_ops.len());
        assert_eq!(
            deserialized.witness_nodes.len(),
            witness.witness_nodes.len()
        );
    ...
    fn test_witness_proof_empty_roundtrip() {
        ...
        assert_eq!(deserialized.new_root_hash, witness.new_root_hash);
        assert_eq!(deserialized.batch_ops.len(), 1);
        assert!(deserialized.witness_nodes.is_empty());
    }
    ...
    fn test_frozen_proof_roundtrip() {
        ...
        let deserialized = crate::v2::api::FrozenProof::from_slice(&buf).unwrap();

        // Verify both proofs validate correctly
        let root_hash = trie.nodestore().root_hash().unwrap();
        proof
            .verify(b"apple", Some(b"red".as_slice()), &root_hash)
            .unwrap();
        deserialized
            .verify(b"apple", Some(b"red".as_slice()), &root_hash)
            .unwrap();
