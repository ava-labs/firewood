...
#[async_trait]
pub trait DbView {
    type Stream: Stream<Item = Result<(Box<[u8]>, Vec<u8>), Error>>;
    ...
    ///
    fn iter_option<K: KeyType>(&self, first_key: Option<K>) -> Result<Self::Stream, Error>;

    /// Obtain a stream over the keys/values of this view, starting from the beginning
    fn iter(&self) -> Result<Self::Stream, Error> {
        self.iter_option(Option::<Box<[u8]>>::None)
    }

    /// Obtain a stream over the key/values, starting at a specific key
    fn iter_from<K: KeyType + 'static>(&self, first_key: K) -> Result<Self::Stream, Error> {
