...
pub(super) use self::children_map::ChildrenMap;
...
mod children_map {
    ...
    #[repr(C)]
    /// A 16-bit bitmap indicating which children are present in a node.
    pub(in crate::proofs) struct ChildrenMap([u8; 2]);
...
mod children_map {
    ...
    #[derive(Clone, Copy, PartialEq, Eq, bytemuck_derive::Pod, bytemuck_derive::Zeroable)]
    #[repr(C)]
    /// A 256-bit bitmap indicating which children are present in a node.
    pub(in crate::proofs) struct ChildrenMap([u8; 32]);
