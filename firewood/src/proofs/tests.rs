...
use firewood_storage::{
    DenseChildren, EthHash, HashMode, HashType, MerkleDbHash, NodeHashAlgorithm, PathComponent,
...
fn test_change_proof_roundtrip(algorithm: NodeHashAlgorithm) {
    ...
    assert_eq!(parsed.hash_mode(), algorithm);
    let mut re_serialized = Vec::new();
    parsed.write_to_vec(&mut re_serialized);
    assert_eq!(serialized, re_serialized);
}
...
#[test_case(magic::MERKLEDB_HASH_MODE; "sha256 hash mode")]
#[test_case(magic::ETHEREUM_HASH_MODE; "keccak hash mode")]
...
fn test_header_validate_accepts_both_hash_modes() {
    ...
    for (byte, expected) in [
        ...
        let validated_header = parse_header(&data)
            .validate(Some(ProofType::Range))
