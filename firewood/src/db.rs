...
mod test {
    ...
    #[test]
    fn test_persist_latest_committed_revision_on_shutdown() {
    ...
    #[test]
    fn test_persist_after_hitting_commit_count() {
        ...
        // Commit SUB_INTERVAL proposals to trigger the first persistence
    ...
    #[test]
    fn test_deferred_persist_with_commit_count_one() {
        const COMMIT_COUNT: usize = 1;
        ...
        // Verify that the proposal was persisted
        let revision = db.revision(root_hash).unwrap();
        let is_persisted = revision
            .root_as_maybe_persisted_node()
            .is_some_and(|node| node.unpersisted().is_none());
        ...
        assert!(
            is_persisted,
            "Root should be persisted when commit count is set to 1"
        );
    }
    ...
    fn test_deferred_persistence_root_store() {
        ...
        // Revisions to verify after reopen (1-indexed commit numbers)
        ...
        // Verify that checkpoint revisions are accessible after reopen
