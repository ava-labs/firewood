...
/// Fuzz test with variable-length keys (1–32 bytes). Short keys create shallow
/// trie structures where out-of-range changes between revisions are more likely
/// to alter branch structure at proof nodes before `start_key`, exercising the
/// child-hash skip logic in `verify_in_range_children`.
#[test]
