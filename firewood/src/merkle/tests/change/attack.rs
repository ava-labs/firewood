...
/// A truncated end proof whose terminal still carries a child at the on-path
...
/// branch leaves a terminal that is a strict ancestor of the bound *and* has a
/// child hash at `b`. Marking that child outside would substitute the proof's
...
fn test_crafted_truncated_end_proof_on_path_child_rejected() {
    ...
    // Change an in-range key so batch_ops is non-empty. 0xfb10 and 0xfb90 are
    // untouched, so child `b` of the [f] branch is present in the end trie.
    let (root1, root2) = setup_2nd_commit!(db, [(b"\xf1", b"A")]);

    let (sk, ek) = (b"\xf0".as_slice(), b"\xfb\x50".as_slice());
    let valid = db
        .change_proof(root1.clone(), root2.clone(), Some(sk), Some(ek), None)
        .unwrap();

    // Truncate the end proof so its terminal is the [f] branch.
    let end_nodes: Vec<crate::ProofNode> = valid
        .end_proof()
        .as_ref()
        .iter()
        .filter(|node| node.key.len() <= 1)
        .cloned()
        .collect();
    let crafted = FrozenChangeProof::new(
        crate::Proof::new(valid.start_proof().as_ref().into()),
    ...
    // Skip structural validation, which would reject the truncation itself.
    let ctx = ChangeProofVerificationContext {
        end_root: root2,
        start_key: Some(sk.to_vec().into()),
        end_key: Some(ek.to_vec().into()),
        right_edge_key: Some(ek.to_vec().into()),
    };

    let parent = db.revision(root1).unwrap();
    let proposal = db.apply_change_proof_to_parent(&crafted, &*parent).unwrap();
    let err = verify_change_proof_root_hash(&crafted, &ctx, &proposal)
        .expect_err("root-hash check must reject a present on-path child");
    assert!(
        matches!(
            err,
            api::Error::ProofError(crate::ProofError::ExclusionProofMissingChild)
        ),
