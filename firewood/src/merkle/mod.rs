...
///
/// Called by [`compute_outside_children`], which applies the marking to the
/// mask and hands the classification back for callers that need it.
...
/// (irrelevant) subtree.
///
/// Classifies the terminal via [`classify_terminal`] and applies the marking.
/// The classification comes back alongside the mask, so no caller can supply one
/// derived from a different `boundary`. When the marking is
/// [`TerminalMarking::OnPath`], only the straddling child's far side is marked
/// here; the change-proof caller uses the returned classification to decide that
/// child itself against the proving trie. Range proofs discard it.
fn compute_outside_children<'a>(
    proof_nodes: &'a [ProofNode],
    boundary: &EdgeBoundary<'_>,
) -> Result<(HashMap<PathBuf, ChildMask>, Option<TerminalClass<'a>>), ProofError> {
    ...
    let terminal = classify_terminal(proof_nodes, boundary);
    ...
    // Terminal node: apply its marking (see `classify_terminal`).
...
fn change_outside_children<S: ReadableStorage, H: HashMode>(
    ...
    // Each edge's mask comes back with its terminal classification, reused for
    // the on-path decision below.
    let (mut outside_children, start_terminal) =
        compute_outside_children(start_proof.as_ref(), &start_boundary)?;
    let (end_children, end_terminal) = compute_outside_children(end_proof.as_ref(), &end_boundary)?;
    ...
    for terminal in [start_terminal, end_terminal].into_iter().flatten() {
        ...
        if proving_merkle.boundary_child_source(terminal_key, on_path, range)?
...
/// the hash of the child frame most recently pushed, so a frame can be suspended
/// and resumed.
struct HashFrame<'b> {
    branch: &'b BranchNode,
    ...
    /// The nibble of the account's lone effective storage child, set only under
...
/// hash and are used directly, except at account depth under `ethhash`. A stored
/// hash there reflects the child count when it was written, and reconcile and
/// collapse can change that count, so the child is read back through `storage`
/// and re-derived. Out-of-range children get their hash from the corresponding
/// proof node.
///
...
/// have been hashed. Re-deriving a persisted child runs this walk once more on
/// that child alone. A node read from storage has only persisted children, all
/// below account depth, so that inner walk takes their stored hashes and never
/// nests further.
///
/// # Errors
///
/// Returns an error if reading a persisted child back from `storage` fails.
fn compute_root_hash_with_proofs<'b, H: HashMode, R: NodeReader>(
    node: &'b Node,
...
/// [`compute_root_hash_with_proofs`] for a node hashed as `hash_as`. The root is
/// always a normal node. A persisted child re-derived at account depth may
/// instead be the account's lone storage child, hashed as a storage-trie root.
fn hash_node_with_proofs<'b, H: HashMode, R: NodeReader>(
    node: &'b Node,
    path_prefix: &[PathComponent],
    hash_as: HashAs,
    proof_nodes: &HashMap<PathBuf, &'b ProofNode>,
    ...
    let Node::Branch(branch) = node else {
        return Ok(hash_leaf_as::<H>(path_prefix, &hash_as, node));
    };
    ...
    loop {
        ...
        // Copies the shared reference out of the frame, so reading children does
        // not extend the mutable borrow of the frame itself.
        ...
        while frame.cursor < BranchNode::MAX_CHILDREN as u8 {
            ...
            if let Child::AddressWithHash(_, hash) | Child::MaybePersisted(_, hash) = child {
                if !at_account_depth {
                    // Away from account depth a stored hash is already the form
                    // this reconstruction needs.
                    frame.child_hashes[nibble] = Some(hash.clone());
                    continue;
                }
                // At account depth the stored hash reflects the child count when
                // it was written, which reconcile and collapse can change. Read
                // the child back and re-derive it.
                let resolved = child.as_shared_node(storage)?;
                let hash_as = child_hash_as::<H>(frame, nibble);
                frame.child_prefix.push(nibble);
                let hash = hash_node_with_proofs::<H, R>(
                    &resolved,
                    &frame.child_prefix[..],
                    hash_as,
                    proof_nodes,
                ...
                frame.child_prefix.pop();
                frame.child_hashes[nibble] = Some(hash);
                continue;
            }
        ...
        match child_node {
            ...
            Node::Leaf(_) => {
                frame.child_hashes[nibble] = Some(hash_leaf_as::<H>(
                    &frame.child_prefix[..],
                    &hash_as,
                    child_node,
                ));
...
/// How a child of `frame` at `nibble` is hashed. The account's lone storage
/// child is folded as a standalone storage-trie root. The decision belongs to the
/// parent, which is the only place that knows the child count.
fn child_hash_as<H: HashMode>(frame: &HashFrame<'_>, nibble: PathComponent) -> HashAs {
    if H::ALGORITHM.is_ethereum() && frame.single_storage_child == Some(nibble) {
...
/// Hashes a leaf in place, as a normal node at `path_prefix` or as a storage-trie
/// root, according to `hash_as`.
fn hash_leaf_as<H: HashMode>(
    path_prefix: &[PathComponent],
    hash_as: &HashAs,
    leaf: &Node,
) -> HashType {
    match hash_as {
        HashAs::Normal => H::to_hash(&HashableShunt::from_node(path_prefix, leaf)),
        HashAs::StorageTrieRoot {
            account_prefix,
            branch_nibble,
        } => {
            hash_node_as_storage_trie_root_for_node::<H>(&account_prefix[..], *branch_nibble, leaf)
        }
    }
}
...
pub fn verify_range_proof<H: ProofCollection<Node = ProofNode>>(
    ...
    match algorithm {
        ...
        NodeHashAlgorithm::MerkleDB => verify_range_proof_root_hash::<H, MerkleDbHash>(
            &all_proof_nodes,
            key_values,
            proof,
            first_key_bytes,
...
fn verify_range_proof_root_hash<P: ProofCollection<Node = ProofNode>, H: HashMode>(
    all_proof_nodes: &[&ProofNode],
    key_values: &[(impl KeyType, impl ValueType)],
    proof: &RangeProof<impl KeyType, impl ValueType, P>,
    first_key_bytes: Option<&[u8]>,
    right_boundary: RightBoundary<'_>,
    root_hash: &TrieHash,
) -> Result<(), api::Error> {
    ...
    for proof_node in all_proof_nodes {
        ...
        proving_merkle.reconcile_branch_proof_node(
            proof_node,
            H::ALGORITHM,
            |pn, branch_value| match &pn.value_digest {
    ...
    let (mut outside_children, _) =
        compute_outside_children(proof.start_proof().as_ref(), &start_boundary)?;
    let (end_children, _) = compute_outside_children(proof.end_proof().as_ref(), &end_boundary)?;
    for (key, flags) in end_children {
    ...
    // Borrow the root rather than taking it by value. See `root_ref` for why.
...
pub fn verify_change_proof_root_hash<H: HashMode>(
    ...
    if let Some(key) = first_proof_key {
        proving_merkle.collapse_root_to_path(key, Some(range))?;
    }

    // Compute which children at each boundary node are outside the proven
    // range, merging both boundaries.
    let outside_children = change_outside_children(proof, verification, &proving_merkle, range)?;
