...
impl<K: MutableKind, S: ReadableStorage, H: HashMode> Merkle<NodeStore<Mutable<K>, S, H>> {
    ...
    fn insert_helper(
        ...
        let result = loop {
            let key_ref: &[u8] = match &owned_key {
                ...
                match node {
                    Node::Branch(ref mut branch) => {
                        let Some(child) = branch.children.take(child_index) else {
                            ...
                            return Ok(InsertStep::Done(node));
                        };
    ...
    fn insert_branch_helper(&mut self, mut node: Node, key: &[u8]) -> Result<Node, FileIoError> {
        ...
        let result = loop {
            ...
            match self.insert_branch_step(node, key_ref)? {
                BranchStep::Done(done) => break done,
                BranchStep::Descend {
                    parent,
                    index,
                    child,
    ...
    fn insert_branch_step(
        ...
        match (
            ...
            (None, None) => match node {
                Node::Branch(_) => Ok(BranchStep::Done(node)),
                Node::Leaf(leaf) => {
                    ...
                    Ok(BranchStep::Done(Node::Branch(Box::new(branch))))
                }
            },
            (None, Some((child_index, partial_path))) => {
                let child_index = PathComponent::try_new(child_index).expect("valid component");
                ...
                match node {
                    Node::Branch(ref mut branch) => {
                        let Some(child) = branch.children.take(child_index) else {
                            ...
                            return Ok(BranchStep::Done(node));
                        };

                        let child = self.read_for_update(child)?;
                        Ok(BranchStep::Descend {
                            parent: node,
                            index: child_index,
                            child,
                            next_key: partial_path,
    ...
    /// descend, so the path is held on the heap rather than the call stack. Trie
    /// depth is attacker-controlled through key length during change-proof
    ...
    /// One level of [`remove_helper`]. Either finishes the removal or hands back the
    /// child to continue into, along with the parent it was taken from.
    ///
    ...
    /// collapse the branch, which can leave nothing behind, hence the `Option`.
    ...
    /// reports whether to descend, so the path is held on the heap rather than the
    /// call stack. Trie depth is attacker-controlled through key length during
    /// change-proof verification, so recursion here would be a stack-exhaustion
    /// sink.
    ...
    /// One level of [`remove_prefix_helper`]. Either finishes the removal or hands
    /// back the child to continue into, along with the parent it was taken from.
    ///
    ...
    /// Deletes all children of a branch node.
    ///
    /// The walk is a worklist rather than recursion. It only tallies what it
    /// frees, so visit order does not matter, and subtree depth follows key
    /// length, which is attacker-controlled during change-proof verification.
    /// Recursing per level would be a stack-exhaustion sink.
    fn delete_children(
        &mut self,
...
/// first. Shared by the iterative insert walks, whose parents are always branches
/// because only the branch arms descend.
fn unwind_parents(mut parents: Vec<(Node, PathComponent)>, mut node: Node) -> Node {
    while let Some((mut parent, index)) = parents.pop() {
        let branch = parent
            .as_branch_mut()
            .expect("only branches are pushed as parents");
        branch.children[index] = Some(Child::Node(node));
        node = parent;
    }
    node
}
