        ...
        for (i, c) in self.chd.iter().enumerate() {
            match c {
                Some(c) => {
                    ...
                    if c_ref.is_encoded_bigger_than_hash_len::<S>(store) {
                        list[i] = Encoded::Data(
...
impl ExtNode {
    fn encode<S: ShaleStore<Node>>(&self, store: &S) -> Vec<u8> {
        ...
        if !self.1.is_null() {
            ...
            if r.is_encoded_bigger_than_hash_len(store) {
                list[1] = Encoded::Data(
                    bincode::DefaultOptions::new()
...
pub struct Node {
    ...
    is_encoded_bigger_than_hash_len: OnceLock<bool>,
...
impl PartialEq for Node {
    fn eq(&self, other: &Self) -> bool {
        let Node {
            root_hash,
            is_encoded_bigger_than_hash_len,
            encoded,
            lazy_dirty,
            inner,
        } = self;
        *root_hash == other.root_hash
            && *is_encoded_bigger_than_hash_len == other.is_encoded_bigger_than_hash_len
...
impl Clone for Node {
    fn clone(&self) -> Self {
        Self {
            ...
            is_encoded_bigger_than_hash_len: self.is_encoded_bigger_than_hash_len.clone(),
...
impl Node {
    ...
    pub(super) fn max_branch_node_size() -> u64 {
        ...
        *max_size.get_or_init(|| {
            Self {
                ...
                is_encoded_bigger_than_hash_len: OnceLock::new(),
    ...
    fn is_encoded_bigger_than_hash_len<S: ShaleStore<Node>>(&self, store: &S) -> bool {
        *self.is_encoded_bigger_than_hash_len.get_or_init(|| {
            self.lazy_dirty.store(true, Ordering::Relaxed);
    ...
    pub(super) fn rehash(&mut self) {
        ...
        self.is_encoded_bigger_than_hash_len = OnceLock::new();
    ...
    pub(super) fn new_from_hash(
        ...
        encoded_bigger_than_hash_len: Option<bool>,
        inner: NodeType,
    ) -> Self {
        Self {
            ...
            is_encoded_bigger_than_hash_len: match encoded_bigger_than_hash_len {
...
impl Storable for Node {
    fn hydrate<T: CachedStore>(addr: usize, mem: &T) -> Result<Self, ShaleError> {
        ...
        let is_encoded_bigger_than_hash_len = if attrs & Node::IS_ENCODED_BIG_VALID == 0 {
            None
        } else {
    ...
    fn dehydrate(&self, to: &mut [u8]) -> Result<(), ShaleError> {
        ...
        attrs |= match self.is_encoded_bigger_than_hash_len.get() {
