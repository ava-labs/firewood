...
impl<'a> std::fmt::Debug for IterationNode<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Unvisited { key, node } => f.debug_struct("Unvisited").field("key", key).field("node", node).finish(),
            Self::Visited { key, children_iter : _} => f.debug_struct("Visited").field("key", key).finish(),
        }
    }
}
