...
use crate::stream::{MerkleKeyValueStream, NodeWithKey, TraversalIterator};
...
impl<T: ReadLinearStore> Merkle<T> {
    ...
    pub fn path_iter<K: KeyType>(
        &self,
        key: K,
    ) -> Result<TraversalIterator<'_, '_, T>, MerkleError> {
...
impl<T: WriteLinearStore> Merkle<T> {
    pub fn insert<K: KeyType>(&mut self, key: K, val: Vec<u8>) -> Result<(), MerkleError> {
    ...
    pub fn insert_and_return_ancestors<K: KeyType>(
        &mut self,
        key: K,
        val: Vec<u8>,
    ) -> Result<Vec<LinearAddress>, MerkleError> {
        let mut traversal_path = TraversalIterator::new(self, key.as_ref())?
...
mod test {
    ...
    fn merkle_build_test<K: KeyType, V: AsRef<[u8]> + Clone>(
        items: Vec<(K, V)>,
