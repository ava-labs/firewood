...
use firewood_storage::{
    BranchNode, Child, FileIoError, Node, PackedPathRef, PartialPath, PathComponent, PathIterItem,
    SharedNode, SplitPath, TriePath, TriePathAsPackedBytes, TriePathFromPackedBytes, TrieReader,
};
use std::cmp::Ordering;
use std::iter::FusedIterator;

/// Helper macro used in iterator `next` implementations to handle returning
/// early on `Err`.
macro_rules! ok {
    ($expr:expr) => {
        match $expr {
...
enum IterationNode {
    ...
    Visited {
        ...
        children_iter: Box<ChildrenIter>,
    },
}
                ...
                IterationNode::Visited {
                    ...
                    let child = match child {
                        ...
                        Child::AddressWithHash(addr, _) => {
                            ok!(self.merkle.read_node(addr))
                        }
                        Child::MaybePersisted(maybe_persisted, _) => {
                            ok!(maybe_persisted.as_shared_node(self.merkle))
                        }
                    };
...
impl NodeIterState {
    fn iter_stack<T: TrieReader>(
        &mut self,
        merkle: &T,
    ) -> Result<&mut Vec<IterationNode>, FileIoError> {
        if let Self::StartFromKey(key) = self {
            *self = get_iterator_intial_state(merkle, key)?;
        }

        let Self::Iterating { iter_stack } = self else {
            unreachable!("we just converted StartFromKey to Iterating");
        };
...
fn get_iterator_intial_state<T: TrieReader>(
    ...
    let Some(mut node) = merkle.root_node() else {
    ...
    let mut matched_key_nibbles = PartialPath::new_const();
    let mut unmatched_key_nibbles = PackedPathRef::path_from_packed_bytes(key);

    loop {
        // See if `node`'s key is a prefix of `key`.
        let partial_path = node.partial_path();
        matched_key_nibbles.extend_from_slice(partial_path);

        let comparison;
        (comparison, unmatched_key_nibbles) =
            compare_partial_path(partial_path, unmatched_key_nibbles);
        ...
        match comparison {
            ...
            Ordering::Equal => match node.as_ref() {
                Node::Leaf(_) => {
                    iter_stack.push(IterationNode::Unvisited {
                        key: matched_key_nibbles,
                        node,
                ...
                Node::Branch(branch) => {
                    let Some(next_unmatched_key_nibble) =
                        unmatched_key_nibbles.split_first_in_place()
                    else {
                    ...
                    iter_stack.push(IterationNode::Visited {
                        key: matched_key_nibbles.clone(),
                        children_iter: ChildrenIter::new(branch).after(next_unmatched_key_nibble),
                    });

                    node = match &branch.children[next_unmatched_key_nibble] {
    ...
    Ok(NodeIterState::Iterating { iter_stack })
}
...
enum PathIteratorState<'a> {
    Iterating {
        ...
        /// prefix of the key we're traversing to.
        matched_key: PartialPath,
        unmatched_key: PackedPathRef<'a>,
        node: SharedNode,
    },
    Exhausted,
}
    ...
    pub(super) fn new(merkle: &'a T, key: &'b [u8]) -> Result<Self, FileIoError> {
        ...
        Ok(Self {
            ...
            state: PathIteratorState::Iterating {
                matched_key: PartialPath::new_const(),
                unmatched_key: PackedPathRef::path_from_packed_bytes(key),
                node: root,
            },
        })
    }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.state {
            ...
            PathIteratorState::Iterating {
                ...
                let partial_path = node.as_ref().partial_path();

                let comparison;
                (comparison, *unmatched_key) = compare_partial_path(partial_path, *unmatched_key);
                ...
                match comparison {
                    ...
                    Ordering::Equal => {
                        matched_key.extend_from_slice(partial_path);
                        let node_key = matched_key.clone();
                        ...
                        match &**node {
                            ...
                            Node::Branch(branch) => {
                                ...
                                let Some(next_unmatched_key_nibble) =
                                    unmatched_key.split_first_in_place()
                                else {
                                ...
                                match child {
                                    ...
                                    Some(Child::AddressWithHash(child_addr, _)) => {
                                        *node = ok!(self.merkle.read_node(*child_addr));
                                    ...
                                    Some(Child::MaybePersisted(maybe_persisted, _)) => {
                                        *node = ok!(maybe_persisted.as_shared_node(self.merkle));
                                        matched_key.push(next_unmatched_key_nibble);
...
fn compare_partial_path<I1: SplitPath, I2: SplitPath>(
    partial_path: I1,
    unmatched_key: I2,
) -> (Ordering, I2) {
    let common = partial_path.longest_common_prefix(unmatched_key);
    let ord = match (common.a_suffix.split_first(), common.b_suffix.split_first()) {
        (None, _) => Ordering::Equal,
        (Some(_), None) => Ordering::Greater,
        (Some((a, _)), Some((b, _))) => Ord::cmp(&a, &b),
    };
    (ord, common.b_suffix)
}

struct ChildrenIter {
    // NB: because `ChildrenSlots` uses arrays internally, it's rather large.
    // (1088 bytes on 64-bit systems, 16-ary trie)
    // (16688 bytes on 64-bit systems, 256-ary trie)
    iter: firewood_storage::ChildrenSlots<Option<Child>>,
    after: Option<PathComponent>,
}

impl ChildrenIter {
    fn new(branch: &BranchNode) -> Box<Self> {
        Box::new(Self {
            iter: branch.children.clone().into_iter(),
            after: None,
        })
    }

    const fn after(mut self: Box<Self>, after: PathComponent) -> Box<Self> {
...
impl Iterator for ChildrenIter {
    type Item = (PathComponent, Child);

    fn next(&mut self) -> Option<Self::Item> {
        self.iter.by_ref().find_map(|(pos, child)| {
            if let Some(after) = self.after {
                if pos > after {
                    // make future next calls slightly faster by skipping this check
                    self.after = None;
                } else {
                    return None;
                }
...
mod tests {
    ...
    macro_rules! assert_path_eq {
        ($a:expr, $b:expr $(,)?) => {
            // match expr used to avoid evaluating both arguments multiple times
            match ($a, PackedPathRef::path_from_packed_bytes($b)) {
                (a, b) => {
                    assert!(
                        a.path_eq(&b),
                        "paths not equal: {} != {}",
    ...
    macro_rules! next {
        ($iter:expr) => {
            $iter.next().expect("unexpected end of iterator").unwrap()
        };
    }
    ...
    fn path_iterate_singleton_merkle(key: &[u8], should_yield_elt: bool) {
        ...
        let mut iter = merkle.path_iter(key).unwrap();
        let Some(node) = iter.next().transpose().unwrap() else {
            assert!(!should_yield_elt);
            return;
        };

        assert!(should_yield_elt);
        assert_path_eq!(node.key_nibbles, &[0xBE, 0xEF]);
    ...
    fn path_iterate_non_singleton_merkle_seek_leaf(key: &[u8]) {
        ...
        let node = next!(iter);
        assert_path_eq!(node.key_nibbles, &[0x00, 0x00, 0x00, 0xFF]);
        assert_eq!(node.next_nibble, None);
    ...
    fn path_iterate_non_singleton_merkle_seek_branch(key: &[u8]) {
        ...
        let node = next!(iter);
        assert_path_eq!(node.key_nibbles, &[0x00, 0x00]);
