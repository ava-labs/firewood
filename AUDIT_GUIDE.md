...
4. **AdversarialProof**: Models every possible combination of Put/Delete/Absent per in-range key in the trie diff, not just combinations that can be generated by an honest generator. It inherits the verification pipeline from ChangeProofVerification and verifies the **OnlyCorrectDiffAccepted** state property. The property requires that whenever verification accepts a change proof, the proposal agrees with the endTrie.
